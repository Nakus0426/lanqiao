...
function disablePayBtn() {
    var $count = parseInt($('.radio').find('em').html());

    if ($count === 0) {
        $('#btn-pay').addClass('graybg-btn');
        $('#btn-pay').find('a').attr('href','javascript:void(0)');
    }else {
        $('#btn-pay').removeClass('graybg-btn');
        $('#btn-pay').find('a').attr('href','order.jsp');
    }
}